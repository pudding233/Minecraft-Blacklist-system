...
if(isset($_POST['submit'])) {
	...
	$note=daddslashes($_POST['note']);
		...
		$sql="update `black_list` set `qq` ='{$qq}',`level` ='{$level}',`note` ='{$note}' ,`source` ='{$source}'where `id`='{$id}'";
	if($DB->query($sql)){
			...
			<div class="form-group">
			 ...
			 <div class="form-group">
              ...
              <div class="col-sm-10"><input type="text" name="note" value="<?php echo $row['note']; ?>" class="form-control"/></div>
                        ...
                        <label class="col-sm-2 control-label">拉黑来源</label>
              <div class="col-sm-10"><input type="text" name="source" value="<?php echo $row['source']; ?>" class="form-control"/></div>
              <br/>
