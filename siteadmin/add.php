...
$source=daddslashes($_POST['source']);
...
if($row!='')exit("<script language='javascript'>alert('后台已存在该黑名单用户！');history.go(-1);</script>");
	$sql="insert into `black_list` (`qq`,`date`,`level`,`note`,`source`) values ('".$qq."','".$date."','".$level."','".$note."','".$source."')";
	$DB->query($sql);
        ...
        <div class="panel-body">
          <form action="./add.php" method="post" class="form-horizontal" role="form">
            <div class="input-group">
              ...
              <input type="text" name="qq" value="<?=@$_POST['qq']?>" class="form-control" placeholder="3F2504E0-4F89-11D3-9A0C-0305E82C3301" autocomplete="off" required/>
            </div><br/>
			<div class="input-group">
            ...
            <div class="input-group">
              ...
              <span class="input-group-addon">拉黑来源</span>
              <input type="text" name="source" value="<?=@$_POST['source']?>" class="form-control" placeholder="无" autocomplete="off" required/>
            </div><br/>
